...
function Header({ nav, page }) {
  ...
  return <header className="top"><button className="brand" onClick={() => nav('home')}><div className="logo">✦</div><span>TestPrep</span></button><nav><button onClick={() => nav('categories')}>Exams</button><button>Features</button><button>Pricing</button><button>Blogs</button><button>About</button></nav><div className="top-actions"><div className="search"><Search size={15}/><input placeholder="Search tests, exams..."/></div><Bell size={18}/>{page === 'home' && <div className="auth-actions"><button className="login-btn" onClick={() => openAuth('login')}>Log In</button><button className="signup-btn" onClick={() => openAuth('register')}>Sign Up</button></div>}<div className="avatar">N</div><span className="name">Student⌄</span></div></header>; }
